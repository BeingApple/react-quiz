...
### 진행하고 있는 테스트
`/pages/index` 를 테스트 합니다. 기본적으로 `퀴즈 풀기`, `오답 노트` 로 이동하기 위한 버튼이 존재하는지 확인하며, 오답노트에 관한 state 가 비어있는 상태로 오답노트 버튼을 클릭할 경우 토스트 문구 `아직 오답노트가 없습니다. 문제를 푼 후 다시 와주세요`가 정상적으로 노출되는지 테스트 합니다.
...
### 해결
`redux-persist` 대신 `window.LocalStorage` 를 사용하도록 변경했습니다. `window` 객체가 아직 로딩 되지 않았거나 존재하지 않는 환경에서의 처리가 조금 번거롭긴 했지만 `_app.tsx` 에선 `useLocalStorage` custom hook을 사용하도록 처리하여 `LocalStorage`에 저장된 오답노트 정보가 항상 `redux` 에 반영되도록 처리하였고 저장될 땐 `window !== undefined`를 체크 후 저장하도록 처리했습니다.
...
하지만, [여러](https://learn-react-test.vlpt.us/#/) [글](https://techblog.woowahan.com/8942/)을 찾아보면서 TDD를 통해 프로젝트의 작동 방식을 보장할 수 있고, 도메인이 복잡해질 수록 잘 만들어진 테스트 케이스가 필요하다는 생각을 했습니다. 또, 테스트 케이스 작동을 고려하다 보니 컴포넌트가 제한적인 책임만을 가지도록 재설계하고 분리하면서 다시 한번 상태와 컴포넌트 설계를 잘 해야겠다는 생각을 하게 됐고, 다음엔 테스트 케이스를 염두한 프로젝트 설계를 해봐야겠습니다.
감사합니다.